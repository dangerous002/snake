...
class createSnake{
    ...
    move(config = this.config, snakeBodyBody = this.generateSnakeBody()){
        ...
        window.addEventListener('keydown', function(e){
            if (e.keyCode == 37 && directionMove != 'right' && directionMove != 'left') {
                directionMove = 'left';
                console.log('left');
            }
            else if (e.keyCode == 38 && directionMove != 'down' && directionMove != 'up') {
                directionMove = 'up';
                console.log('up');
            }
            else if (e.keyCode == 39 && directionMove != 'left' && directionMove != 'right') {
                console.log('right');
                directionMove = 'right';
            }
            else if (e.keyCode == 40 && directionMove != 'up' && directionMove != 'down') {
                console.log('down');
                directionMove = 'down';
            }
        });

        let controlButtons = document.querySelector('.control-buttons').childNodes;

        for(let i = 1; i < controlButtons.length; i++){
            controlButtons[i].addEventListener('click', function(event){
                //event.target.parentElement
                if(event.target.parentElement.getAttribute('id') == 'control-button-up' && directionMove != 'down' && directionMove != 'up'){
                    console.log('up');
                    directionMove = 'up';
                } else if (event.target.parentElement.getAttribute('id') == 'control-button-left' && directionMove != 'right' && directionMove != 'left'){
                    console.log('left');
                    directionMove  = 'left';
                } else if (event.target.parentElement.getAttribute('id') == 'control-button-down' && directionMove != 'up' && directionMove != 'down'){
                    console.log('down');
                    directionMove = 'down';
                } else if (event.target.parentElement.getAttribute('id') == 'control-button-right' && directionMove != 'left' && directionMove != 'right'){
                    directionMove = 'right';
                }
            });
        }
    ...
    if(parseInt(document.getElementById('elementsX').value) >= 10 && parseInt(document.getElementById('elementsX').value) <= 40 && parseInt(document.getElementById('elementsY').value) >= 10 && parseInt(document.getElementById('elementsY').value) <= 40){
        ...
        */
        ...
        setTimeout(function(){
            ...
            document.querySelector('.control-buttons').style.opacity = '1';
